...
paths <- data$Pathogen_Name_short
paths_1 <- c("B. cereus", "B. subtilis",
            "E. faecalis", "M. luteus",
            "M. smegmatis", "N. corynebacteroides",
            "S. aureus", "S. epidermidis",
            "A. baumanii", "C. freundii",
            "E. cloacae", "E. coli",
            "K. oxytoca", "P. vulgaris",
            "P. aeruginosa (PAO1)", "P.aeruginosa 27873",
            "S. marcescens", "A. flavus",
            "C. albicans", "Candida spp.",
            "C. neoformans", "T. asahii")
...
data$Pathogen_Name_short <- factor(data$Pathogen_Name_short, levels = paths)
...
theatmap <- theatmap[, paths_1]
theatmap <- theatmap %>% select(all_of(paths_1)) #this code also works for the same function above
...
annotation_col = data.frame(Pathogen_Type = data$Pathogen_Type, Pathogen_Name = data$Pathogen_Name_short)
