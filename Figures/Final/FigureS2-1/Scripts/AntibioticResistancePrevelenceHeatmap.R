...
Iso.Meta <-as.data.frame(read_csv("./IsolateGenomes_Metadata.csv"))
IsoCount<- as.data.frame(read.csv("./IsolatePhylumSpeciesCount.csv"))

Nudge.df <- merge(Nudge, Iso.Meta, by = "sample", all = T)
...
ISOResults.FIN.df <- merge(ISOResults, IsoCount, by ="Species", all = T)
...
ISOResults.Isolate.freq <- ISOResults.FIN.df %>% group_by(Species,SPECIES_TOTAL) %>% 
  dplyr::count(AMRclass) %>% 
  mutate(freq = n / SPECIES_TOTAL * 100) # divided by the 34 subjects

Species.Matrix <- acast(ISOResults.Isolate.freq, AMRclass ~ Species, value.var="freq")
Species.Matrix[is.na(Species.Matrix)] <- 0

rownames(IsoCount) <-IsoCount$Species
IsoCount.matrix<- IsoCount[-1]
