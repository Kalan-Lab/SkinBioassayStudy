...
dir5 = 'Bowtie2_Mappings_EPIC_and_SMGC/' # not used
dir6 = 'Bowtie2_Mappings_EPIC_with_Cacnes/'
dir7 = 'Bowtie2_Mappings_EPIC_SMGC_and_EMAGs/'
dir8 = 'Bowtie2_Mappings_EPIC_IsoGenomes_and_MAGs/'
...
data7 = {}
for f in os.listdir(dir7):
    mg = f.split('.')[0].replace('_', '-')
    with open(dir7 + f) as odf:
        for line in odf:
            line = line.strip()
            if '% overall alignment rate' in line:
                data7[mg] = line.split('% overall alignment rate')[0]

data8 = {}
for f in os.listdir(dir8):
    mg = f.split('.')[0].replace('_', '-')
    with open(dir8 + f) as odf:
        for line in odf:
            line = line.strip()
            if '% overall alignment rate' in line:
                data8[mg] = line.split('% overall alignment rate')[0]
